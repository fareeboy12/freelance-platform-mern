    ...
    <Box>
        <Box key={jobData?._id} sx={{ borderBottom: '1px solid #e4ebe4', py: 5, px: 5, '&:hover': {backgroundColor: '#f2f7f2',}}}>
            <Box sx={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between' }}>
                <Box sx={{ width: '80%' }}>
                    <Link href="#" underline="hover">{jobData?.jobTitle}</Link>
                </Box>
                ...
                <Box sx={{ width: '100%' }}>
                    <Typography variant='body1' sx={{ fontSize: '12px' }}>Fixed-price - Expert - Est. Budget: ${jobData?.projectBudget} - Posted {displayJobCreationTime(jobData?.createdAt)}</Typography>
                </Box>
